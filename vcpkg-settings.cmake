...
macro (setup_vcpkg)
    ...
    if (USE_PROJECT_VCPKG)
        ...
        if (NOT EXISTS "${CMAKE_CURRENT_LIST_DIR}/archives")
            string(APPEND _storage_directories "archives ")
            execute_process (
        ...
        if (NOT EXISTS "${CMAKE_CURRENT_LIST_DIR}/buildtrees")
            string(APPEND _storage_directories "buildtrees ")
            execute_process (
        ...
        if (NOT EXISTS "${CMAKE_CURRENT_LIST_DIR}/downloads")
            string(APPEND _storage_directories "downloads ")
            execute_process (
        ...
        if (NOT EXISTS "${CMAKE_CURRENT_LIST_DIR}/installed")
            string(APPEND _storage_directories "installed ")
            execute_process (
        ...
        if (NOT EXISTS "${CMAKE_CURRENT_LIST_DIR}/packages")
            string(APPEND _storage_directories "packages ")
            execute_process (
    ...
    if (NOT DEFINED VCPKG_DEFAULT_TRIPLET)
        set (VCPKG_DEFAULT_TRIPLET "$ENV{VCPKG_DEFAULT_HOST_TRIPLET}" CACHE STRING "")
    endif ()
    if (NOT DEFINED VCPKG_DEFAULT_HOST_TRIPLET)
        set (VCPKG_DEFAULT_HOST_TRIPLET "$ENV{VCPKG_DEFAULT_HOST_TRIPLET}" CACHE STRING "")
    endif ()
    if (NOT DEFINED VCPKG_DEFAULT_TRIPLET)
    ...
    if (NOT DEFINED VCPKG_MANIFEST_FEATURES)
        set (VCPKG_MANIFEST_FEATURES "c" "cpp" CACHE STRING "This variable can be set to a list of features to activate when installing your manifest. For example, features can be used by projects to control building with additional dependencies.")
    endif ()
    option (VCPKG_MANIFEST_NO_DEFAULT_FEATURES "This variable controls activation of default features in additon to those listed in 'VCPKG_MANIFEST_FEATURES'. If set to 'ON', default features will not be automatically activated." OFF)
    ...
    if (NOT DEFINED VCPKG_FEATURE_FLAGS)
        set (VCPKG_FEATURE_FLAGS
            ...
            # Specifies where downloaded tools and source code archives shoudl be kept.
    ...
    # option (VCPKG_PREFER_SYSTEM_LIBS "This variable controls whether vcpkg will append instead of prepend it's paths to 'CMAKE_PREFIX_PATH', 'CMAKE_LIBRARY_PATH', and 'CMAKE_FIND_ROOT_PATH' so that vcpkg libraries/packages are found after toolchain/system libraries/packages. Defaults to 'OFF'." ON)
